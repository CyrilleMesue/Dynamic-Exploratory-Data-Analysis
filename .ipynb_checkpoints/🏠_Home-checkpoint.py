...
data_format = part0col1.radio("Select input file format", ["CSV", "TSV", "EXCEL"], index =0)

# Option 1 : Load data from a file in either csv, smi or text format
uploaded_file = part0col1.file_uploader("Please upload data in csv format", accept_multiple_files=False, type = ['csv', "tsv", "xlsx"])
    ...
    if uploaded_file == None:
        ...
        try:
            if data_format == "CSV":
                ...
                uploaded_data = pd.read_csv(uploaded_file,header=0,encoding='utf-8', sep = "\t")
            else:
            ...
            part0col1.warning("Warning! Uploaded file format must match selected file format!")
